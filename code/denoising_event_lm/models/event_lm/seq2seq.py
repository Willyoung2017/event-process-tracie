...
class EventLMTransformerSeq2Seq(ModelBase):
    ...
    def __init__(self,
        ...
        self._event_token_id = self._tokenizer.convert_tokens_to_ids(EVENT_TAG)

        self._pointer_event_token_ids = [self._tokenizer.convert_tokens_to_ids(tag) for tag in POINTER_EVENT_TAGS if tag in self._tokenizer.additional_special_tokens]

        self._arg_token_id = self._tokenizer.convert_tokens_to_ids(ARGS_TAG)

...
class EventSeq2SeqTransformerReader(Seq2SeqTransformerReader):
    ...
    def _read(self, file_path: str):
        ...
        all_instances = []
        if not os.path.exists("all_gpt2_instances.pkl"):
            for example in dataset:
                ...
                self.set_task_specific_args(self._default_task_specific_args)
            with open("all_instances.pkl", "wb") as f:
            ...
            with open("all_gpt2_instances.pkl", "rb") as f:
    ...
    def make_single_instance(
        ...
        order = sorted(list(range(len(keep_varg_seq))), key=lambda x: permutation[x])
        gold_varg_seq = [keep_varg_seq[i] for i in order]

        prev_or_next_prob = -0.30
        if random.random() < prev_or_next_prob:
            if random.random() < 0.5:
                keep_varg_seq = gold_varg_seq[:-1]
                gold_varg_seq = [gold_varg_seq[-1]]
                source_prefix = self._source_prefix +  " next event:"
            else:
                keep_varg_seq = gold_varg_seq[1:]
                gold_varg_seq = [gold_varg_seq[0]]
                source_prefix = self._source_prefix + " previous event:"
        else:
            source_prefix = self._source_prefix +  " reorder:"
        ...
        source_str = source_prefix + source_str
        source_encodes = self.tokenize_text(source_str)

        # get the gold chain, and target_str
        # if not self._test_mode:
        #     assert is_neg_chain is None and aug_metadata is None

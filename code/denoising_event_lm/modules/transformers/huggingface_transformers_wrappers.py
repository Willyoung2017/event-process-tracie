...
def get_huggingface_tokenizer(pretrained_model_name_or_path, *inputs, **kwargs):
    ...
    for extra_tok_id in range(len(additional_special_tokens)):
        if not tokenizer.convert_tokens_to_ids(additional_special_tokens[extra_tok_id]) == tokenizer.vocab_size + extra_tok_id:
    ...
    def __init__(
        ...
        if transformer_name.split('/')[-1].startswith("bart"):
            ...
            if transformer_name.startswith("gpt2"):
                if seq2seq_type == "standard":
                    seq2seq_type =  GPT2LMHeadModel
                else:
                    raise ValueError("Unsupported lm type for LMHeadModel: {}".format(seq2seq_type))
                self._base_model = seq2seq_type.from_pretrained(
                    transformer_name,
